...
cosmic.run <- function(df, sample.column) {
  ...
  if (all(cosmic.final2[,c("CHROM","POS","REF","ALT",sample.column)]==df[,c("CHROM","POS","REF","ALT",sample.column)])) {
    print("good")
    MUTS <- cbind(df, cosmic.final2 %>% dplyr::select(CosmicCount,heme_cosmic_count,myeloid_cosmic_count))
    return(MUTS)
  } else {
    print("bad")
  }
...
args <- parse_args(parser, list(c("-i","/Users/brian/test/1239160_23153_0_0.final.tsv"),
                                c("-o","/Users/brian/Bolton/UKBB/results/lung/update/1239160_23153_0_0.final.reannotate.tsv"),
                                ...
                                c("-v","~/Bolton/data/bick.bolton.vars.txt"),
...
final.test.lung.ran <- read.table(args$input, sep = "\t", header = T, quote = "", comment.char = "")
new.lung <-  read.table("/Users/brian/test/6022994_23153_0_0.final.tsv", sep = "\t", header = T, quote = "", comment.char = "")
cols.to.add <- setdiff(colnames(new.lung), colnames(final.test.lung.ran))
cols.to.remove <- setdiff(colnames(final.test.lung.ran), colnames(new.lung))
...
vardict.pon2 <- read.table("/storage1/fs1/bolton/Active/projects/mocha/UKBB/exome/young/vardict/filtered/vardict.2N.tsv", 
                           sep = "\t", header = F, quote = "", comment.char = "")
colnames(vardict.pon2) <- c("CHROM","POS","REF","ALT")
vardict.pon2$Vardict_PON_2AT2_percent <- 1
mutect.pon2 <- read.table("/storage1/fs1/bolton/Active/projects/mocha/UKBB/exome/young/mutect2/mutect2.merged.SB.2N.tsv", 
                          sep = "\t", header = F, quote = "", comment.char = "")
...
final.df <-  left_join(final.test,
                       annotation %>%
                         dplyr::select("CHROM","POS","REF","ALT","SAMPLE","isOncogenic","isTSG",
                                       "CosmicCount","heme_cosmic_count","myeloid_cosmic_count","MDS",
