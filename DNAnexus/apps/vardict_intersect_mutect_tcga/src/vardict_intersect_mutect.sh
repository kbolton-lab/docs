...
main() {
    ...
    export name="$eid_nameroot".vardict.intersect.mutect.vcf.gz
    ...
    /usr/bin/bcftools isec -n+2 -w1 complex.variant.query.vcf.gz ${intersect_vcf_path} -Oz -o $eid_nameroot.vardict.intersect.mutect.no.complex.vcf.gz && /usr/bin/tabix $eid_nameroot.vardict.intersect.mutect.no.complex.vcf.gz
    
    /usr/bin/bcftools concat -a -D complex.only.variant.query.vep.annotated.vcf.gz $eid_nameroot.vardict.intersect.mutect.no.complex.vcf.gz -Oz -o $name
    /usr/bin/tabix $name

    intersect_vcf=$(dx upload $name --brief)
    intersect_vcf_index=$(dx upload $name.tbi --brief)

    dx-jobutil-add-output intersect_vcf "$intersect_vcf" --class=file
    dx-jobutil-add-output intersect_vcf_index "$intersect_vcf_index" --class=file
}
