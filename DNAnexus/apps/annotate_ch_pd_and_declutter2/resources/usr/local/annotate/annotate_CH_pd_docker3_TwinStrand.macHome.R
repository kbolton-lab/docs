...
cosmic.run <- function(df, sample.column, var_key.column) {
  ...
  # X.1 <- lapply(ls(X1), function(x) {
    ...
    # cosmic <- read.table(paste0("~/test/CosmicMutantExport.final.more.minimal.test.",x,".tsv"),
...
# write.table(setNames(data.frame(matches.2.c.p), "exact_match"),
#             "~/Bolton/data/matches.2.c.p.txt", quote = F, row.names = F)
...
args <- parse_args(parser, list(c("-i","~/Bolton/data/hg38_mut_full_long_filtered_KB_deid_2.tsv"),
    ...
    c("-v", paste("/Volumes/bolton/Active/projects/TwinStrand/PRJ00087.2021.05.24.deliverables/results/mutect_unannotated/final.mutect.sorted_annotated.vcf.gz",
                  "/Volumes/bolton/Active/projects/TwinStrand/PRJ00087.2021.05.24.deliverables/results/pindel_unannotated/final.pindel.sorted_annotated.vcf.gz",
                  "/Volumes/bolton/Active/projects/TwinStrand/PRJ00087.2021.05.24.deliverables/results/mutect_pon_unannotated/final.mutect_pon.sorted_annotated.vcf.gz",
                  "/Volumes/bolton/Active/projects/TwinStrand/PRJ00087.2021.05.24.deliverables/results/vardict_unannotated/final.vardict.sorted_annotated.vcf.gz",
                  "/Volumes/bolton/Active/projects/TwinStrand/PRJ00087.2021.05.24.deliverables/results/varscan_unannotated/final.varscan.sorted_annotated.vcf.gz",
                  "/Volumes/bolton/Active/projects/TwinStrand/PRJ00087.2021.05.24.deliverables/results/lofreq_unannotated/final.lofreq.sorted_annotated.vcf.gz",
                  sep=",")),
                                ...
                                # c("-p","~/Bolton/data/pd_table_kbreview_bick_trunc2.txt"), # new
                                c("-p","~/Bolton/data/pd_table_kbreview_bick_trunc2.txt"), # new
                                c("--pan-myeloid","~/Bolton/data/panmyeloid_variant_counts.vep.annotated.vcf.tsv"), # new
                                c("--number-samples-annot",5),
                                c("--blacklist","~/Bolton/data/ENCFF356LFX.bed"),
                                c("--segemental-duplications","~/Bolton/data/dup.grch38.bed.gz"),
                                c("--simple-repeats","~/Bolton/data/simpleRepeat.bed"),
                                c("--repeat-masker","~/Bolton/data/repeatMaskerJoinedCurrent.bed"),
                                c("--target-length", 39652397), ## 38997831, ## TwinStrand 
                                c("--bolton-bick-vars","~/Bolton/data/bick.bolton.vars3.txt"), # new
                                c("--mut2-bick","~/Bolton/data/topmed.n2.mutation.c.p.txt"), # new
                                c("--mut2-kelly","~/Bolton/data/kelly.n2.mutation.c.p.txt"),
                                c("--matches2","~/Bolton/data/matches.2.c.p.txt"), # MORE NEW!
                                # c("--matches2","/Users/brian/test/matches.2.c.p.txt"), # MORE NEW!
                                c("--col-order","/Users/brian/Bolton/UKBB/results/lung/update/1218265_23153_0_0.columns.txt"))) # new
...
if (is.na(args$impact_file) || is.na(args$bick_file) || is.na(args$cosmic_file) || 
    ...
    is.na(args$bolton_bick_vars) || is.na(args$mut2_bick) || is.na(args$mut2_kelly)) {
...
mutect2_vcf_file <- variant.files[which(grepl("final.mutect.sorted_annotated.vcf.gz", variant.files))]
mutect2_pon_vcf_file <- variant.files[which(grepl("final.mutect_pon.sorted_annotated.vcf.gz", variant.files))]
vardict_vcf_file <- variant.files[which(grepl("final.vardict.sorted_annotated.vcf.gz", variant.files))]
varscan_vcf_file <- variant.files[which(grepl("final.varscan.sorted_annotated.vcf.gz", variant.files))]
pindel_vcf_file <- variant.files[which(grepl("final.pindel.sorted_annotated.vcf.gz", variant.files))]
lofreq_vcf_file <- variant.files[which(grepl("final.lofreq.sorted_annotated.vcf.gz", variant.files))]
...
mutect2.df <- mutect2[["dat"]]

mutect2_pon <- vcfR2tidy(read.vcfR(mutect2_pon_vcf_file, verbose = FALSE),
                     single_frame = TRUE,
                     info_types = TRUE,
                     format_types = TRUE)
mutect2_pon.info <-mutect2_pon$meta[mutect2_pon$meta$Tag=="INFO",]$ID
mutect2_pon.info <- mutect2_pon.info[!(mutect2_pon.info %in% c("SAMPLE","PON_RefDepth", "PON_AltDepth", "PON_FISHER", "CSQ"))]
mutect2_pon.df <- mutect2_pon[["dat"]]
...
varscan <- vcfR2tidy(read.vcfR(varscan_vcf_file, verbose = FALSE),
                     single_frame = TRUE,
                     info_types = TRUE,
                     format_types = TRUE)
varscan.info <- varscan$meta[varscan$meta$Tag=="INFO",]$ID
varscan.info <- varscan.info[!(varscan.info %in% c("OLD_MULTIALLELIC", "SAMPLE", "CSQ"))]
varscan.df <- varscan[["dat"]]
varscan.df <- varscan.df[,!colnames(varscan.df) %in% c("OLD_MULTIALLELIC")]

pindel <- vcfR2tidy(read.vcfR(pindel_vcf_file, verbose = FALSE),
                     single_frame = TRUE,
                     info_types = TRUE,
                     format_types = TRUE)
pindel.info <- pindel$meta[pindel$meta$Tag=="INFO",]$ID
pindel.info <- pindel.info[!(pindel.info %in% c("OLD_MULTIALLELIC", "SAMPLE", "CSQ"))]
test.pindel <- pindel[["dat"]]
pindel.df <- pindel[["dat"]]
pindel.df <- pindel.df[,!colnames(pindel.df) %in% c("OLD_MULTIALLELIC")]

lofreq <- vcfR2tidy(read.vcfR(lofreq_vcf_file, verbose = FALSE),
                    single_frame = TRUE,
                    info_types = TRUE,
                    format_types = TRUE)
lofreq.info <- lofreq$meta[lofreq$meta$Tag=="INFO",]$ID
lofreq.info <- lofreq.info[!(lofreq.info %in% c("OLD_MULTIALLELIC", "SAMPLE", "CSQ"))]
lofreq.df <- lofreq[["dat"]]
lofreq.df <- lofreq.df[,!colnames(lofreq.df) %in% c("OLD_MULTIALLELIC")]


mutect2.df$skey <- with(mutect2.df, paste(CHROM,POS,REF,ALT,SAMPLE, sep = ":"))
mutect2_pon.df$skey <- with(mutect2.df, paste(CHROM,POS,REF,ALT,SAMPLE, sep = ":"))
setdiff(mutect2.df$skey, mutect2_pon.df$skey)

vardict.df$skey <- with(vardict.df, paste(CHROM,POS,REF,ALT,SAMPLE, sep = ":"))
varscan.df$skey <- with(varscan.df, paste(CHROM,POS,REF,ALT,SAMPLE, sep = ":"))
pindel.df$skey <- with(pindel.df, paste(CHROM,POS,REF,ALT,SAMPLE, sep = ":"))
lofreq.df$skey <- with(lofreq.df, paste(CHROM,POS,REF,ALT,SAMPLE, sep = ":"))
...
mutect2.df <- mutect2.df %>% separate(CSQ, paste0(CSQnames, "_VEP"), sep="\\|", extra = "merge", fill = "right")
vardict.df <- vardict.df %>% separate(CSQ, paste0(CSQnames, "_VEP"), sep="\\|", extra = "merge", fill = "right")
varscan.df <- varscan.df %>% separate(CSQ, paste0(CSQnames, "_VEP"), sep="\\|", extra = "merge", fill = "right")
pindel.df <- pindel.df %>% separate(CSQ, paste0(CSQnames, "_VEP"), sep="\\|", extra = "merge", fill = "right")
lofreq.df <- lofreq.df %>% separate(CSQ, paste0(CSQnames, "_VEP"), sep="\\|", extra = "merge", fill = "right")

mutect2.df <- mutect2.df[!duplicated(mutect2.df),]
# mutect2_pon.df <- mutect2_pon.df[!duplicated(mutect2_pon.df),]
vardict.df <- vardict.df[!duplicated(vardict.df),]
varscan.df <- varscan.df[!duplicated(varscan.df),]
pindel.df <- pindel.df[!duplicated(pindel.df),]
lofreq.df <- lofreq.df[!duplicated(lofreq.df),]
...
mutect2.df <- mutect2.df %>% separate(gt_AD, c("gt_AD_ref","gt_AD_alt"), 
                                    sep=",",extra = "merge", fill = "right")
mutect2.df$gt_AD_ref <- as.numeric(mutect2.df$gt_AD_ref)
mutect2.df$gt_AD_alt <- as.numeric(mutect2.df$gt_AD_alt)
vardict.df <- vardict.df %>% separate(gt_AD, c("gt_AD_ref","gt_AD_alt"), 
                                      sep=",",extra = "merge", fill = "right")
varscan.df <- varscan.df %>% separate(gt_AD, c("gt_AD_ref","gt_AD_alt"), 
                                      sep=",",extra = "merge", fill = "right")
pindel.df <- pindel.df %>% separate(gt_AD, c("gt_AD_ref","gt_AD_alt"), 
                                      sep=",",extra = "merge", fill = "right")
lofreq.df <- lofreq.df %>% separate(DP4, c("gt_AD_ref_forw","gt_AD_ref_rev","gt_AD_alt_forw","gt_AD_alt_rev"), 
                                      sep=",",extra = "merge", fill = "right")
lofreq.df$gt_AD_ref <- lofreq.df$gt_AD_ref_forw + lofreq.df$gt_AD_ref_rev
...
final.passed <- sqldf("SELECT l.*, r.`truncating_loci_total`, r.`source.totals.loci.truncating`
            FROM `final.passed` as l
            LEFT JOIN `vars.truncating` as r
            on l.key = r.key OR l.gene_loci_vep = r.gene_loci_vep") # source.totals.loci
final.passed <- final.passed[!duplicated(final.passed),]
final.passed$truncating_loci_total <- fillna(final.passed$truncating_loci_total, 0)
...
annotate.PD <- function(x) {
  ...
  # test <- as.data.frame(A) %>% filter(VariantClass=="Frame_Shift_Del"|
  ...
  # test$Gene
  # 
  # test2 <- as.data.frame(A) %>% filter(VariantClass=="Missense_Mutation" & Exon=="*") %>%
  #   dplyr::select(Gene) %>%
  #   distinct()
  # 
  # write.table(setdiff(test$Gene,test2$Gene),"~/103.genes.in.4.nonsense.classes.not.in.messense.review.txt",
