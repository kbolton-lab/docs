...
/usr/bin/bcftools view -s ${tumor_sample_name} --threads 64 $eid_nameroot.vardict.vcf.gz | /usr/bin/bcftools norm -f $REF -m -any --threads 64 | /usr/bin/bcftools filter -i "((FMT/AF * FMT/DP < 6) && ((FMT/MQ < 55.0 && FMT/NM > 1.0) || (FMT/MQ < 60.0 && FMT/NM > 2.0) || (FMT/DP < 10) || (FMT/QUAL < 45)))" -m+ -s "BCBIO" --threads 64 -Oz -o $eid_nameroot.vardict.BCBIOfiltered.vcf.gz && /usr/bin/tabix $eid_nameroot.vardict.BCBIOfiltered.vcf.gz 