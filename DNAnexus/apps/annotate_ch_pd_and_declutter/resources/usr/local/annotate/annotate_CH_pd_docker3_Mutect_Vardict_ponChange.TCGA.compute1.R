...
# if (is.na(args$variant_calls_files) || is.na(args$TSG_file) || is.na(args$eid) || 
#     is.na(args$oncoKB_curated) || is.na(args$pd_annotation_file) || is.na(args$target_length) ||
...
# tumor
# mutect2_vcf_file <- "/Volumes/bolton/Active/projects/TCGA/vcfs/C239.TCGA-04-1331-01A-01W.5_gdc_realn.mutect.vep.annotated.vcf.gz"
vardict_vcf_file <- variant.files[which(grepl("vardict", variant.files))]
# tumor
# vardict_vcf_file <- "/Volumes/bolton/Active/projects/TCGA/vcfs/C239.TCGA-04-1331-01A-01W.5_gdc_realn.vardict.BCBIOfiltered.vardict.final.annotated.vcf.gz"
...
write.table(mutect2.final, paste0("/scratch1/fs1/bolton/brian/",args$sample_id,".mutect.tmp"), row.names = F, quote = F, sep = "\t")
mutect2.final <- read.table(paste0("/scratch1/fs1/bolton/brian/",args$sample_id,".mutect.tmp"), sep = "\t", header = T, quote = "")
write.table(vardict.final, paste0("/scratch1/fs1/bolton/brian/",args$sample_id,".vardict.tmp"), row.names = F, quote = F, sep = "\t")
vardict.final <- read.table(paste0("/scratch1/fs1/bolton/brian/",args$sample_id,".vardict.tmp"), sep = "\t", header = T, quote = "")
system(command = paste0("rm /scratch1/fs1/bolton/brian/",args$sample_id,".mutect.tmp"), intern = T)
system(command = paste0("rm /scratch1/fs1/bolton/brian/",args$sample_id,".vardict.tmp"), intern = T)
...
blacklist <- rtracklayer::import("/storage1/fs1/bolton/Active/projects/annotation_files/ENCFF356LFX.bed")
print(blacklist)
message("###########################                      E")
...
#   filter(n >=2)
# 
# truncating_genes <- unique(vars.truncating$Gene)
# truncating <- c("frameshift_variant", "splice_region_variant", "stop_gained")
# 
# final.passed$isTruncatingHotSpot <- ifelse(final.passed$SYMBOL_VEP %in% truncating_genes & final.passed$Consequence_VEP %in% truncating, 1, 0)
...
annotate.PD <- function(x) {
  ...
  # MUTS$oncoKB <- system(command = paste0('curl -X POST -H "Content-Type: application/json" -d',
  #                                        "'", jsonlite::toJSON(unname(json_query), auto_unbox = T), "'",
  #                                        ' "https://www.oncokb.org/api/v1/annotate/mutations/byProteinChange" -H "Authorization: Bearer da03f096-284a-490a-8b3f-95c5f1bf5666" | jq -r ".[].oncogenic"'), 
  #                       intern = T)
  write(jsonlite::toJSON(unname(json_query), auto_unbox = T), "~/shit.json")
  system(command = 'curl -X POST -H "Content-Type: application/json" -d@$HOME/shit.json https://www.oncokb.org/api/v1/annotate/mutations/byProteinChange -H "Authorization: Bearer da03f096-284a-490a-8b3f-95c5f1bf5666" | jq ".[].oncogenic" > ~/shit.return.tsv', 
                        intern = T)
  system(command = "awk -F'\t' '$2=\"dummy\" {print}' OFS='\t' ~/shit.return.tsv > ~/shit.return.tsv.t && mv ~/shit.return.tsv.t ~/shit.return.tsv", intern = T)
  MUTS$oncoKB <- read.table("~/shit.return.tsv", quote='"', sep = "\t", header = F)$V1
  ...
  truncating_genes <- unique(vars.truncating$Gene)
  MUTS$isTruncatingHotSpot <- ifelse(final.passed$SYMBOL_VEP %in% truncating_genes & final.passed$Consequence_VEP %in% truncating, 1, 0)
  MUTS <- MUTS %>%
...
# message(class(final.df$oncoKB))
colnames(final.df)[(which(sapply(final.df, class)=="list"))]
# final.df$oncoKB.test <- sapply(final.df$oncoKB, function(x) as.list(x))
