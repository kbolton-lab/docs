...
args <- parse_args(parser, list(c("-i","~/Bolton/data/hg38_mut_full_long_filtered_KB_deid_2.tsv"),
                                ...
                                # c("--pan-myeloid","~/Bolton/data/panmyeloid_variant_counts.hg38.tsv"),
                                ...
                                c("--simple-repeats","/Users/brian/Bolton/data/simpleRepeat.bed"),
                                c("--repeat-masker","/Users/brian/Bolton/data/repeatMaskerJoinedCurrent.bed"),
...
## figure out how to only do this for columns that are not from Alex's script
...
#   dplyr::mutate(loci = ifelse(grepl('[A-Z]\\d+',AAchange),str_extract(AAchange, '[A-Z]\\d+'), AAchange)) %>%
#   dplyr::group_by(Gene, loci) %>%
#   mutate(source = paste(unique(str_split(paste0(source, collapse = ","),",")[[1]]),collapse=",")) %>%
#   dplyr::mutate(n=dplyr::n()) %>%
#   dplyr::ungroup() %>%
#   dplyr::filter(n >= num) %>%
#   #distinct(CHROM,POS,REF,ALT, .keep_all = TRUE) %>%
...
## if False means at least 1 of groups has a Max VAF > .0007 so this is stringent for filtering out
...
## all 3 needs to have a Max above .0007
...
annotate.PD <- function(x) {
  ...
  translate_consequence = c(
    ...
    "stop_gained" = "Nonsense_Mutation", # A nonsense mutation is a genetic mutation in a DNA sequence that results in a shorter, unfinished protein product
    ...
    "stop_lost" = "Nonsense_Mutation", # = not shorter but longer
  ...
  # MUTS$HGVSp_VEP <- gsub(".*.:", "", MUTS$HGVSp_VEP)
  MUTS$HGVSp_VEP2 <- gsub("(.*p\\.)(.*)", "\\2", MUTS$HGVSp_VEP)
  MUTS$HGVSp_VEP2 <- gsub("(.*)(fs.*)", "\\1", MUTS$HGVSp_VEP2)
  MUTS$HGVSp_VEP2 <- gsub("(.*)(extTer.*)", "\\1", MUTS$HGVSp_VEP2)
  MUTS$HGVSp_VEP2 <- gsub("del", "", MUTS$HGVSp_VEP2)
  MUTS$HGVSp_VEP2 <- gsub("%3D", "=", MUTS$HGVSp_VEP2) 
  
  MUTS$HGVSp_VEP3 <- gsub("fsTer", "fs*", MUTS$HGVSp_VEP)
  MUTS$HGVSp_VEP3 <- gsub("(.*p\\.)(.*)", "\\2", MUTS$HGVSp_VEP3)
  AminoAcids = c('Cys'= 'C', 'Asp'= 'D', 'Ser'= 'S', 'Gln'= 'Q', 'Lys'= 'K',
    'Ile'= 'I', 'Pro'= 'P', 'Thr'= 'T', 'Phe'= 'F', 'Asn'= 'N',
    'Gly'= 'G', 'His'= 'H', 'Leu'= 'L', 'Arg'= 'R', 'Trp'= 'W',
    'Ala'= 'A', 'Val'='V', 'Glu'= 'E', 'Tyr'= 'Y', 'Met'= 'M',
  ...
  for (i in 1:length(AminoAcids)) {
    MUTS$HGVSp_VEP3 <- gsub(names(AminoAcids)[i], AminoAcids[i], MUTS$HGVSp_VEP3)
  }
  
  MUTS <- MUTS[,c("CHROM","POS","REF","ALT","SYMBOL_VEP","HGVSp_VEP","HGVSp_VEP2","HGVSp_VEP3","VariantClass","SAMPLE","EXON_VEP","eid")]
  
  # MUTS$aa_ref <- sapply(MUTS$HGVSp_VEP2, function(x) str_split(x, "[0-9]+", n=2)[[1]][1])
  MUTS$aa_ref <- sapply(MUTS$HGVSp_VEP3, function(x) str_split(x, "[0-9]+", n=2)[[1]][1])
  MUTS$aa_alt <- sapply(MUTS$HGVSp_VEP3, function(x) str_split(x, "[0-9]+", n=2)[[1]][2])
  # MUTS$aa_alt.extra <- sapply(MUTS$HGVSp_VEP3, function(x) str_split(x, "[0-9]+", n=2)[[1]][2]) ## always with have either 1) Amino Acid, 2) "dup", 3) "=", 3) AA and "fs*"
  # MUTS$aa_alt.extra <- sapply(MUTS$HGVSp_VEP3, function(x) str_split(x, "[0-9]+", n=2)[[1]][2])
  MUTS$aa_pos <- as.numeric(str_extract(MUTS$HGVSp_VEP3, "\\d+"))
  ...
  MUTS$AAchange[grep("[A-Z]{1}[0-9]{1,5}[A-Z]{1}", MUTS$AAchange)[1:1000]][988]
  MUTS$HGVSp_VEP3[grep("[A-Z]{1}[0-9]{1,5}[A-Z]{1}", MUTS$AAchange)[1:1000]][988]
  MUTS[988,]
  
  MUTS$AAchange[which(!(grepl("[A-Z]{1}[0-9]{1,5}[A-Z]{1}", MUTS$AAchange)[1:100]))]
  ...
  # MUTS.chr1 <- MUTS %>% filter(CHROM=="chr1")
  cosmic.final.chr1.chr17 <- cosmic.final %>% 
    dplyr::select(GENOMIC_MUTATION_ID, 
                  Accession.Number,
                  HGVSG,
                  HGVSC,
                  HGVSP,
                  ...
                  myeloid_count_chr,
                  var_key)
  test %>% filter(var_key == "chr1:43352302:C:T")
  ptm <- proc.time()
  test.cosmic <- sqldf("SELECT l.*, r.GENOMIC_MUTATION_ID, r.cosmic_count_chr, r.haematopoietic_and_lymphoid_tissue_count_chr, r.myeloid_count_chr, r.`Accession.Number`
         FROM `MUTS` as l
         LEFT JOIN `cosmic.final` as r
         on l.var_key = r.var_key OR l.HGVSp_VEP = r.HGVSP")
  
  colnames(cosmic.final) <- c("ID","var_key","HGVSP","CosmicCount","heme_cosmic_count","myeloid_cosmic_count")
  test.cosmic <- sqldf("SELECT l.*, r.var_key, HGVSP, r.CosmicCount, r.heme_cosmic_count, r.myeloid_cosmic_count
         FROM `MUTS` as l
         LEFT JOIN `cosmic.final` as r
         on l.var_key = r.var_key OR l.HGVSp_VEP = r.HGVSP")
  proc.time() - ptm
  test.chr1.join <- test %>% filter(CHROM=="chr1")
  
  
  MUTS.chr1 = left_join(MUTS.chr1, cosmic.final %>% 
                          dplyr::select(GENOMIC_MUTATION_ID, 
                                        cosmic_count_chr,
                                        haematopoietic_and_lymphoid_tissue_count_chr, 
                                        HGVSP, var_key), 
                        by="var_key")
  MUTS.chr1 %>% filter(cosmic_count_chr>=1)
  ...
  test.pan <- panmyeloid_variant_counts %>% filter(Annotation == "SNP" | Annotation == "ONCOGENIC")
  diff <- setdiff(unique(test.pan$SYMBOL_VEP), unique(A$Gene))
  test.pan.final <- test.pan %>% 
    filter(SYMBOL_VEP %in% diff, !is.na(SYMBOL_VEP)) %>%
    mutate(total = MDS + AML + MPN) %>%
    group_by(SYMBOL_VEP) %>%
    summarise(sum_total = sum(total)) %>%
    arrange(desc(sum_total))
  as.data.frame(test.pan.final[test.pan.final$sum_total>=10,1])
  t <- test.pan %>% 
    filter(SYMBOL_VEP == "GATA2") %>%
    mutate(total = MDS + AML + MPN)
    
    aggregate(by=sum, FUN=sum)
  sort(table(panmyeloid_variant_counts$SYMBOL_VEP[panmyeloid_variant_counts$SYMBOL_VEP %in% diff]), decreasing = T)
  ...
  get_oncokb = function(mut) {
    ...
    request_url = paste0("https://oncokb.org//api/v1/annotate/mutations/byProteinChange?hugoSymbol=",
                         mut['SYMBOL_VEP'], "&alteration=", mut['HGVSp_VEP3'], "&consequence=", mut['VariantClass'])
...
final.df <-  left_join(final.passed,
                       annotation %>%
                         dplyr::select("CHROM","POS","REF","ALT","SAMPLE","CosmicCount","heme_cosmic_count","MDS",
                                       "AML","MPN","oncoKB","isOncogenic","isTSG","ch_my_pd","ch_pd","ch_pd2","VariantClass","AAchange","Gene"),
