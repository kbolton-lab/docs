...
mutect2.final <- read.table(paste0("/scratch1/fs1/bolton/brian/",args$sample_id,".mutect.tmp"), 
...
vardict.final <- read.table(paste0("/scratch1/fs1/bolton/brian/",args$sample_id,".vardict.tmp"), 
                            sep = "\t", header = T, quote = "", comment.char = "")
...
annotate.PD <- function(x) {
  ...
  A.star <- A[A$VariantClass=="missense_variant",]
  t <- data.frame(Gene=unique(c(TSG, A.star$Gene)))
  t$`Missense variants` <- ifelse(t$Gene %in% A.star$Gene, "x", "")
  t$`Tumor suppressor` <- ifelse(t$Gene %in% TSG, "x", "")
  write.table(t, "Bolton/HIV/supp_table.tsv", row.names = F, quote=F, sep = "\t")
  A.star.missense.review <- unique(A.star$Gene[A.star$ch_my_pd==2])
  A.star.missense.patho <- unique(A.star$Gene[A.star$aa_ref!="*" & A.star$aa_pos!="*" & A.star$aa_alt!="*"])
  length(A.star.missense.patho)
  length(A.star.missense.review)
  length(intersect(A.star.missense.patho, A.star.missense.review))
  ## about ~68 genes
  # ch.my.pd
  ...
  MUTS <- MUTS %>% 
    mutate(WHY_CH_ch_my_pd = ifelse(MUTS$ch_my_pd>0 | MUTS$ch_pd>0, "PD_table;", ""),
           WHY_CH_ch_pd = ifelse(ch_pd==1, "Truncating & ch_pancan_pd;", ""))
  ...
  MUTS <- MUTS %>%
    mutate(ch_pd = ifelse(oncoKB=="Oncogenic" | oncoKB=="Likely Oncogenic" | oncoKB=="Predicted Oncogenic", 1, ch_pd),
           WHY_CH_ch_pd = ifelse(oncoKB=="Oncogenic" | oncoKB=="Likely Oncogenic" | oncoKB=="Predicted Oncogenic", 
                                 paste0(WHY_CH_ch_pd, " OncoKB;"), WHY_CH_ch_pd))
