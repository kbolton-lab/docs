...
cosmic.run <- function(df, sample.column="SAMPLE", var_key.column="var_key") {
  ...
  X.1 <- lapply(ls(X1), function(x) {
    ...
    cosmic <- read.table(paste0("/Volumes/bolton/Active/projects/annotation_files/cosmic/CosmicMutantExport.final.minimal.",x,".tsv"),
    ...
    colnames(cosmic) <- c("COSMIC_ID","tumor_id","var_key","HGVSP","CosmicCount","heme_cosmic_count","myeloid_cosmic_count","Gene_HGVSp_VEP")
...
parser <- add_argument(parser, "--mutect2-vcf-file", type="character", help="merged file")
...
### read in       
mutect2_vcf_file <- args$variant_calls_files
...
annotate.PD <- function(x) {
  ...
  # new_gene_census <- RT("/Users/brian/Bolton/data/Census_allMon Jul 11 22_32_37 2022.tsv")
  # colnames(new_gene_census)[c(1,15)] <- colnames(gene_census)
  # new_gene_census.tsg <- new_gene_census %>% filter(grepl("TSG", Role.in.Cancer))
  # new_gene_census.og <- new_gene_census %>% filter(grepl("oncogene", Role.in.Cancer))
  # new_oncoKB <- RT("/Users/brian/Bolton/data/cancerGeneList.tsv")
  # colnames(new_oncoKB) <- gsub("\\.","_",colnames(new_oncoKB))
  # colnames(new_oncoKB)[10] <- "Is_Tumor_Suppressor"
  # new_oncoKB_curated.tsg = new_oncoKB %>% filter(Is_Tumor_Suppressor=="Yes")
  # 
  # new_oncoKB_curated.og = new_oncoKB %>% filter(Is_Oncogene=="Yes")
  # 
  # length(unique(c(gene_census$Gene, oncoKB_curated.tsg$Hugo_Symbol)))
  # new_gene_census.tsg.final <- rbind(gene_census, new_gene_census.tsg[,colnames(gene_census)]) %>% distinct(Gene, .keep_all = T)
  # WT(new_gene_census.tsg.final, "~/Bolton/data/gene_census_TSG_updated.txt")
  # new_oncoKB_curated.tsg.final <- rbind(oncoKB_curated.tsg[,intersect(colnames(oncoKB_curated.tsg), gsub("\\.","_",colnames(new_oncoKB_curated.tsg)))],
  #                                       new_oncoKB_curated.tsg[,intersect(colnames(oncoKB_curated.tsg), gsub("\\.","_",colnames(new_oncoKB_curated.tsg)))]) %>% 
  #   distinct(Hugo_Symbol, .keep_all = T)
  # WT(new_oncoKB_curated.tsg.final, "~/Bolton/data/all_curated_genes_v2.0_updated.tsv")
  # length(unique(c(new_gene_census.tsg.final$Gene, new_oncoKB_curated.tsg.final$Hugo_Symbol)))
  # length(unique(c(new_gene_census.og$Gene, new_oncoKB_curated.og$Hugo_Symbol)))
  
  
  # # gene_census no longer annotated as TSG
  # sapply(list(gene_census$Gene, new_gene_census.tsg$Gene.Symbol), length)
  # setdiff(gene_census$Gene, new_gene_census.tsg$Gene)
  # setdiff(new_gene_census.tsg$Gene, gene_census$Gene)
  # 
  # # gene_census no longer annotated as Oncogenes
  # sapply(list(gene_census.og$Gene, new_gene_census.og$Gene.Symbol), length)
  # setdiff(gene_census.og$Gene, new_gene_census.og$Gene.Symbol)
  # 
  # 
  # 
  # # oncoKB no longer annotated as TSG
  # sapply(list(oncoKB_curated.tsg$Hugo_Symbol, new_oncoKB_curated.tsg$Hugo.Symbol), length)
  # setdiff(oncoKB_curated.tsg$Hugo_Symbol, new_oncoKB_curated.tsg$Hugo.Symbol)
  # 
  # # oncoKB no longer annotated as Oncogenes
  # sapply(list(oncoKB_curated.og$Hugo_Symbol, new_oncoKB_curated.og$Hugo.Symbol), length)
  # setdiff(oncoKB_curated.og$Hugo_Symbol, new_oncoKB_curated.og$Hugo.Symbol)
