...
function part2(df,finalgroup,pp)
    ...
    for j in 1:length(p[!,1])
        ...
        for k in 1:length(finalgroup[2]) # each group
            if 0 <= poss[finalgroup[2][k][1][1]] - p.POS[j] <= length(p.ALT[j])  # check if pindel is larger than complex and within 15 bp of start of complex
...
function submain(df,p,oname,P)
    ...
    # xx=
    # if isnothing(xx)
    #     CSV.write(oname,DataFrame(x1=["nothing found"],x2=[0,0]),delim="\t",compress=false)
    #     return -1
    # end
    CSV.write(oname,submain2(groupsfun(copy(df),p),P),delim="\t")
    return 0
...
function submain2(d,P)
    ...
    if ! isnothing(d) && nrow(d)>0
        ...
        P.calpos = [missing for i in 1:nrow(P)]
        P.calref = [missing for i in 1:nrow(P)]
        P.calalt = [missing for i in 1:nrow(P)]
        P.altcount = [missing for i in 1:nrow(P)]
        P.refcount = [missing for i in 1:nrow(P)]
        P.binary = [missing for i in 1:nrow(P)]
        P.calpos_best = [missing for i in 1:nrow(P)]
        P.calalt_best = [missing for i in 1:nrow(P)]
        P.calref_best = [missing for i in 1:nrow(P)]
    end
