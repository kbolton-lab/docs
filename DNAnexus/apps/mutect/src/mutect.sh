...
postprocess() {
    ...
    # extract tumor
    # /usr/bin/bcftools view -s $tumor_sample_name -Oz -o $tumor_sample_name.$output_name ${output_name} && /usr/bin/tabix $tumor_sample_name.$output_name

    ## concat, normalize, and extract tumor
    /usr/bin/bcftools concat --allow-overlaps --remove-duplicates -Oz --threads 8 /tmp/mutect/*.vcf.gz | /usr/bin/bcftools view -s $tumor_sample_name | bcftools norm -m -any --threads 8 -Oz -o $tumor_sample_name.normalized.$output_name && /usr/bin/tabix $tumor_sample_name.normalized.$output_name

    vcf=$(dx upload "$tumor_sample_name.normalized.$output_name" --brief)
    dx-jobutil-add-output vcf --class=file "$vcf" 

    vcf_index=$(dx upload "$tumor_sample_name.normalized.$output_name.tbi" --brief)
    dx-jobutil-add-output vcf_index --class=file "$vcf_index"
