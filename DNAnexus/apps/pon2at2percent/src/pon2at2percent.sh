...
main() {
    ...
    /usr/bin/bcftools annotate -a normal2.txt.gz -h pon2.header -c CHROM,POS,REF,ALT,PON_2AT2_percent $vcf -Oz -o $name
    tabix $name
    ...
    dx-jobutil-add-output tumor_only_vcf "$annotated_pon2_vcf" --class=file
    dx-jobutil-add-output tumor_only_vcf_index "$annotated_pon2_vcf_index" --class=file
}
