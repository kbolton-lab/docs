...
main() {
    ...
    bcftools +fill-tags --threads 4 output.filtered.norm.tp53.vcf.gz -- -t F_MISSING,TYPE,HWE,VAF | \
        bcftools filter --threads 4 -e 'INFO/F_MISSING>=0.1' -m+ -s "F_MISSING" | \
        bcftools filter --threads 4 -e 'INFO/HWE<=1e-15' -m+ -s "HWE" | \
        bcftools filter --threads 4 -e 'INFO/F_MISSING>=0.1' -m+ -s "F_MISSING"
    
    bcftools norm -m -any /storage1/fs1/bolton/Active/projects/mocha/UKBB/exome/germline/HaplotypeCaller2/output.vcf.gz | \
        bcftools +fill-tags --threads 4 -- -t F_MISSING,TYPE,HWE,VAF | \
        bcftools filter --threads 4 -i 'CHROM="chr17" && POS>=7668421 && POS<=7687490' | \
        bcftools filter --threads 4 -e 'INFO/F_MISSING>=0.1' -m+ -s "F_MISSING" | \
        bcftools filter --threads 4 -e 'INFO/HWE<=1e-15' -m+ -s "HWE" | \
        bcftools filter --threads 4 -e '(INFO/TYPE="SNP" && (MAX(FMT/DP)<7 || MAX(VAF)<=0.15))' -m+ -s "SNP" | \
        bcftools filter --threads 4 -e '(INFO/TYPE="INDEL" && (MAX(FMT/DP)<10 || MAX(VAF)<=0.20))' -m+ -s "INDEL" |
        bcftools query -f '%CHROM:%POS:%REF:%ALT\t%FILTER\n' | grep -v PASS | wc -l

    /storage1/fs1/bolton/Active/projects/mocha/UKBB/ukbb_calls/pvcf/test_tp53$ \
        bcftools view -S /storage1/fs1/bolton/Active/projects/mocha/UKBB/exome/germline/HaplotypeCaller2/eids.txt /storage1/fs1/bolton/Active/projects/mocha/UKBB/ukbb_calls/pvcf/test_tp53/ukb23156_c17_b8_v1.vcf.gz | \
            bcftools norm -m -any | \
            bcftools +fill-tags --threads 4 -- -t F_MISSING,TYPE,HWE,VAF | \
            bcftools filter --threads 4 -i 'CHROM="chr17" && POS>=7668421 && POS<=7687490' | \
            bcftools filter --threads 4 -e 'INFO/F_MISSING>=0.1' -m+ -s "F_MISSING" | \
    ...
    bcftools norm -m -any /storage1/fs1/bolton/Active/projects/mocha/UKBB/exome/germline/HaplotypeCaller2/output.vcf.gz | \
        bcftools +fill-tags --threads 4 -- -t F_MISSING,TYPE,HWE,VAF | \
        bcftools filter --threads 4 -i 'CHROM="chr17"' | \
        bcftools filter --threads 4 -e 'INFO/F_MISSING>=0.1' -m+ -s "F_MISSING" | \
        bcftools filter --threads 4 -e 'INFO/HWE<=1e-15' -m+ -s "HWE" | \
        bcftools filter --threads 4 -e '(INFO/TYPE="SNP" && (MAX(FMT/DP)<7 || MAX(VAF)<=0.15))' -m+ -s "SNP" | \
        bcftools filter --threads 4 -e '(INFO/TYPE="INDEL" && (MAX(FMT/DP)<10 || MAX(VAF)<=0.20))' -m+ -s "INDEL" |
