...
workflow=workflow-GFjqGYQJP6z5Y90b0yzqjB24
...
dx find executions -n4000 --project $NEW_EXOME --state in_progress --origin-jobs | cut -d ' ' -f2 | grep -v "^$" | cut -d- -f2 | sort > /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/in_progress.txt
...
for batch in {0000..0018}; do
   ...
   if [ $word_count -gt 1 ]; then
      dx run $workflow --batch-tsv /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$folder/dx_batch.$batch.tsv   -y --priority low --extra-args '{"executionPolicy": {"restartOn": {"UnresponsiveWorker": 2}, "maxSpotTries": 2}}'
   else
   echo no
   fi
done

curl http://repo.gsc.wustl.edu/files/mgi_ca.pem >> /etc/ssl/certs/certificates.crt
...
# GERMLINE
folder=50
workflow=workflow-GBYK2Z8JP6z0z7k1Px5b8PK7
...
for i in {17..60}; do;
   ...
   gsed -i "s/folder=50/folder=$i/g" /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$i/batch.sh
