...
workflow=workflow-GBYK2Y0JP6zGFFZg2jBgyPkZ
workflow=workflow-GFYzyX0JP6zKVYzQ9pKYBZ2Z
...
dx find executions -n3000 --project $NEW_EXOME --state in_progress --origin-jobs | cut -d ' ' -f2 | grep -v "^$" | cut -d- -f2 | sort > /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/in_progress.txt
...
for i in {17..60}; do;
   ...
   gsed -i "s/folder=45/folder=$i/g" /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$i/batch.sh
