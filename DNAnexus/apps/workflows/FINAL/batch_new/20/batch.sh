...
workflow=workflow-GBX0210JP6z6YKy5BYJfvYpP
...
folder=20
workflow=workflow-GBX0210JP6z6YKy5BYJfvYpP
dx run $workflow --batch-tsv /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$folder/dx_batch.germline.tsv -y --priority low --extra-args '{"executionPolicy": {"restartOn": {"UnresponsiveWorker": 2}, "maxSpotTries": 5}}' --instance-type 0=mem1_ssd1_v2_x16 --instance-type 2=mem2_ssd1_v2_x4

for folder in $(dx ls $NEW_EXOME:Brian/Workflow_Outputs); do
   dx mv "$NEW_EXOME:Brian/Workflow_Outputs/$folder/19/20*" "$NEW_EXOME:Brian/Workflow_Outputs/$folder/20/"
done
...
for i in {17..60}; do;
   ...
   gsed -i "s/folder=20/folder=$i/g" /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$i/batch.sh
