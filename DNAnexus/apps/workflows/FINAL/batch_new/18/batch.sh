...
workflow=workflow-GBX01B0JP6z9k7Xf59Q3Q63Y
...
for batch_file in $(ls dx_batch.000{0..2}.tsv); do num=$(echo $batch_file | cut -d. -f2); cat both.txt samples.original.txt /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/in_progress.txt | grep -v -f /dev/stdin dx_batch.$num.tsv > dx_batch.$num.tsv.tmp && mv dx_batch.$num.tsv.tmp dx_batch.$num.tsv; done
...
for folder in {41..60}; do
   echo $folder
   cd /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$folder
   gsed -i 's/G9yjZQjJQ285yb258VF90Z2B/GBVzz0QJQ2800VfZ3Fxqp0JQ/' dxworkflow.json
   dx build --workflow . --destination project-G8YxJ2QJP6z69ZVVFVfBYyPB:/Brian/workflows/
   echo; echo;
done
# 0000
...
for i in {17..60}; do;
   ...
   gsed -i "s/folder=18/folder=$i/g" /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$i/batch.sh
