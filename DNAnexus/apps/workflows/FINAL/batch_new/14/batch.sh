...
workflow=workflow-GB1GQp0JP6zPyjKx6yZZG6z5
...
for folder in {14..60}; do
   echo $folder
   cd /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$folder
   wc -l dx_batch.00**.tsv | tail -n1
(head -n1 dx_batch.0000.tsv; grep -f germline.ids.$folder.txt dx_batch.00**.tsv | cut -d: -f 2-) > dx_batch.germline.tsv
   for batch_file in $(ls dx_batch.00*.tsv); do 
      num=$(echo $batch_file | cut -d. -f2); 
      grep -v -f germline.ids.$folder.txt dx_batch.$num.tsv > dx_batch.$num.tsv.tmp && mv dx_batch.$num.tsv.tmp dx_batch.$num.tsv;
   done
   wc -l dx_batch.*.tsv | tail -n1
   echo
done
...
## single
name=1498590
dx run $workflow -istage-G4Q4Pk8J6XGFjX9F3xfbg8Qv.bam=ffile-G560yxjJykJqfK3B4pg5P3fx -istage-G4Q4Pk8J6XGFjX9F3xfbg8Qv.bam_index=file-G9yjf4jJP6z7F1Vk6zQjV8q1 -y --priority low --extra-args '{"executionPolicy": {"restartOn": {"UnresponsiveWorker": 2}, "maxSpotTries": 5}}' --instance-type 0=mem3_ssd1_v2_x8 --instance-type 2=mem2_ssd1_v2_x4 --name "$name"

dx run $workflow -istage-G4Q4Pk8J6XGFjX9F3xfbg8Qv.bam=file-G9yjf08JP6zF11QPPpzJFB6P -istage-G4Q4Pk8J6XGFjX9F3xfbg8Qv.bam_index=file-G560zv0JykJvqx7X4x43gZ93 -y --priority low --extra-args '{"executionPolicy": {"restartOn": {"UnresponsiveWorker": 2}, "maxSpotTries": 5}}' --instance-type 0=mem3_ssd1_v2_x8 --instance-type 2=mem2_ssd1_v2_x4 --name "$name"
...
bsub -oo sort.log -G compute-bolton -g /bwileytest -q general -M 16G -R 'select[mem>16G] rusage[mem=16G]' -a 'docker(ubuntu)' /bin/bash -c "sort new_germline.all.key.txt | uniq -c | sort -k1,1rn > new_germline.all.key.count.txt"
...
for i in {17..60}; do;
   ...
   gsed -i "s/folder=14/folder=$i/g" /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch_new/$i/batch.sh
