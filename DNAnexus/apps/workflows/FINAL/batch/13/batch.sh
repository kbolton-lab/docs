...
for folder in 12; do
    ...
    for batch in dx_batch.0002.tsv; do
        ...
        for file in split_${batch}_*; do
            ...
            mv -f ${batch}_tmp_file "$file"
            wc -l $file
            dx run $workflow_id --batch-tsv $file -istage-G4fq8KjJ6XG6pBYqK1VXFgZV.project=$BRIAN -y --priority low --extra-args '{"executionPolicy": {"restartOn": {"UnresponsiveWorker": 2}, "maxSpotTries": 2}}'
            sleep 30
        done
    done
done