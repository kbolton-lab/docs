...
args <- parse_args(parser, list(c("-T","/Volumes/bolton/Active/projects/annotation_files/gene_census_TSG.txt"),
                                ...
                                c("--output","/Volumes/bolton/Active/projects/mocha/UKBB/exome/results/germline/already_ran/1002297_23153_0_0.updated.tsv"),
                                c("-i","/Volumes/bolton/Active/projects/mocha/UKBB/exome/results/germline/already_ran/1002297_23153_0_0.final.tsv")))
library(tidyverse)
...
cols.rem <- c("COSMIC_ID","CosmicCount","heme_cosmic_count","myeloid_cosmic_count",
...
cols.rem2 <- c("n.loci.vep","source.totals.loci","n.HGVSp","source.totals.p",
              "n.HGVSc", "source.totals.c")
input <- input %>% dplyr::select(-cols.rem2[cols.rem2 %in% colnames(input)])
...
cosmic.run <- function(df, sample.column="SAMPLE", var_key.column="var_key") {
  ...
  # X.1 <- lapply(ls(X1), function(y) {
    ...
    cosmic <- read.table(paste0("/Volumes/bolton/Active/projects/annotation_files/cosmic/CosmicMutantExport.final.more.minimal.test.",y,".tsv"),
