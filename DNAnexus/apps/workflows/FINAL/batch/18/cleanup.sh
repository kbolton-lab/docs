...
## check all QC
tail -n+2 cleanup/Alignment_QC.txt | cut -d, -f4 | cut -d_ -f1 | sort | uniq > QC.total
...
############## Recheck there are 4*3909 QC, 1*4002 MSK, & 2*3909 Mutect/Vardict ##############
wc -l $folder.crams.run
7900
########### compute1 ###########
...
bsub -oo mutect.log -G compute-bolton -q general -M 4G -R 'rusage[mem=4G]' -a 'docker(kboltonlab/dnanexus:1.0)' /bin/bash ./download.sh 18 MutectVEP
bsub -oo vardict.log -G compute-bolton -q general -M 4G -R 'rusage[mem=4G]' -a 'docker(kboltonlab/dnanexus:1.0)' /bin/bash ./download.sh 18 VardictIntersectMutect
#!/bin/bash
# download.sh
DX2
folder=$1
