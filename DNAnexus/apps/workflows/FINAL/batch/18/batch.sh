...
folder=18
PROJECT=$KELLY
workflow=workflow-G7ZBPY0JQ28KjZ3f4VK57qX5
...
for batch in {0000..0003} {0000..0007}; do
   for file in a b; do
      ...
      if [ $word_count -gt 1 ]; then
         dx run $workflow --batch-tsv /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch/$folder/dx_batch.$batch.$file.tsv -y --priority low --extra-args '{"executionPolicy": {"restartOn": {"UnresponsiveWorker": 2}, "maxSpotTries": 2}}' --instance-type 0=mem3_ssd1_v2_x8 --instance-type 5=mem2_ssd1_v2_x8
      else
      echo no
      fi
   done
