...
dx ls $BRIAN:/CH_Exome/Workflow_Outputs/FINAL/Final_Outputs/12/"*.final.tsv" | cut -d_ -f1 | sort | uniq | grep "^12") > brian.12.txt
...
dx ls $KELLY:/CH_Exome/Workflow_Outputs/FINAL/Final_Outputs/12/"*.final.tsv" | cut -d_ -f1 | sort | uniq | grep "^12") > kelly.12.txt


wc -l /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch/12/FINAL_COMBINE_JOBS_12/dx_batch.000*.tsv

for file in $(ls dx_batch.000*.tsv); do
    name=$(basename $file .tsv)
    grep -v -f brian.12.txt $file | grep -v -f kelly.12.txt > $name.remaining.tsv
done
...
dx run workflow-G6Q7gZ0JQ281kBv3GQxFGf5g --batch-tsv /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch/12/FINAL_COMBINE_JOBS_12/dx_batch.0002.remaining.tsv -istage-G4KqPz0J6XGJZGB842qJVYQK.project=$KELLY -y --priority low --extra-args '{"executionPolicy": {"restartOn": {"UnresponsiveWorker": 2}, "maxSpotTries": 5}}'
