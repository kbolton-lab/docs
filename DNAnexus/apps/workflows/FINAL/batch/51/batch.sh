...
721 Bams so far (77 WGA and 644 D-DNA)
5790 singlets of D-DNA
5790 - 644 = 5146 bams to download
cd /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch/TCGA
for uuid in $(grep -v -f blood_bams blood_normal.D_DNA.tsv | awk '{print $1}' | tail -n+3 | head -n999); do
   dx run $JIE:/test/tools/tcga_dl -y -iuuid=$uuid --destination $JIE:/CH_Exome/Inputs/TCGA/BAMS/Blood_Test/ --name tcga_dl-${uuid}
done
########################################################################################################
########################################################################################################
...
folder=51
PROJECT=$BRIAN
workflow=workflow-G7xjQ5jJ6XGF43KX4F69kyKb
...
cp /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch/51/batch.sh /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch/53/batch.sh
cp /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch/51/dxworkflow.json /Users/brian/Bolton/UKBB/docs/DNAnexus/apps/workflows/FINAL/batch/53/dxworkflow.json
