...
process() {
    ...
    docker run --rm -v /home/dnanexus:/home/dnanexus -v /mnt/UKBB_Exome_2021:/mnt/UKBB_Exome_2021 -v /usr/local/bin/:/usr/local/bin -w /home/dnanexus kboltonlab/vardict:bedtools \
        /bin/bash /usr/local/bin/VardictJava.sh ${reference_path} ${AF_THR} ${tumor_bam_path} ${tumor_sample_name} ${interval_file_path} ${normal_bam_path} ${normal_sample_name} ${shard}.vardict.vcf ${interval_file_name}

    vardict_text=$(dx upload ${interval_file_name}.txt --brief --destination project-G4qpk1jJQ285yvbXPFZKXkk8:/Brian/test/)
    # dx upload $vardict_text --destination 
...
postprocess() {
    ...
    # extract tumor
    /usr/bin/bcftools concat --allow-overlaps --remove-duplicates --threads 8 /tmp/vardict/*.vcf.gz | /usr/bin/bcftools view -s ${tumor_sample_name} --threads 8 | bcftools norm -f $reference_name -m -any --threads 8 -Oz -o $output_name && /usr/bin/tabix $output_name
