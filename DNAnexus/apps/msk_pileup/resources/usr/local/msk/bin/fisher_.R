...
df <- read.table("~/test/1331501_23153_0_0.fisher.input", header=F)
...
df1 <- df[3100,]
df1$fisher.exact.pval <- apply(df1, 1, function(x) {
  x <- as.numeric(x[-c(1,2,3,4)])
  if (x[2]==0 & x[1]!=0) {
    return(0)
  } else if ((x[1]==0 & x[2]!=0) | (x[3]==0 & x[4]!=0)) {
    return(1) 
  ## mutect can have 0,0 depth for uninformative reads. # x[1]+x[2] == 0 | 
  } else if (x[1]+x[2] == 0 | x[3]+x[4] == 0) {
    return(1)
